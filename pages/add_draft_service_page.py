...
class AddDraftService(BasePage):
...
#  Scenario: I go to service page and add a draft service
    ...
    def select_draft_radio_btn(self):
        self.driver.find_element(*self.DRAFT_RADIO_BTN).click()
        sleep(3)

        #de verificat daca save e enabled, si dupa sa faca click
        #ca altfel trece testul chiar daca e disbled
        #nu merge asa. tot timpul da ca e ok
        #sau daca pun aasert false, imi da tot timpul failed la acel pas
    # def test_element_not_displayed_v1(self):
    #     try:
    #        self.driver.find_element(*self.SAVE_SERVICE_MODAL_BTN).is_displayed()
    #        assert ("Elementul nu a fost gasit")
    ...
    #lista = //section/div[1]/div[2]/div
    # LISTA_SERVICII = (By.XPATH, "//section/div[1]/div[2]/div")
    # lista = driver.find_elements(*self.LISTA_SERVICII)
    # print(len(lista))
    ...
    def verify_the_no_of_services(self):
        list = self.driver.find_elements(By.XPATH, "//section/div[1]/div[2]/div")
        print(len(list))
